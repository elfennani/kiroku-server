...
pub struct MediaProcessorRepository {
    db: Arc<Database>,
}

impl MediaProcessorRepository {
    pub fn new(db: Arc<Database>) -> MediaProcessorRepository {
        Self { db }
    }

    async fn save_chapters(&self, metadata_id: usize, chapters: Vec<Chapter>) -> Result<()> {
        let mut conn = self.db.conn.lock().await;

        for chapter in chapters {
            sqlx::query!(
                "INSERT INTO metadata_chapters (metadata_id, title, start, end, `index`) VALUES (?, ?, ?, ?, ?)",
                metadata_id as i64,
                chapter.title,
    ...
    async fn save_audio_streams(
        ...
        for stream in streams {
            sqlx::query!(
                "INSERT INTO metadata_streams (metadata_id, title, language, `index`, `type`) VALUES (?, ?, ?, ?, ?)",
                metadata_id as i64,
                stream.title,
                stream.language,
                stream.index as i64,
    ...
    async fn save_subtitle_streams(
        &self,
        metadata_id: usize,
    ...
    async fn save_metadata(&self, media_metadata: MediaMetadata) -> Result<i64> {
        let id = {
            let conn = self.db.conn.lock().await;

            let res = sqlx::query!(
                "INSERT INTO metadata (title, duration) VALUES (?, ?)",
